...
# --- 2. LOGIC วิเคราะห์ข้อความ (Strict Mode) ---
def process_message(target_id, text, is_admin_sender):
    # 1. ลบช่องว่างออกก่อน เพื่อให้จับแพทเทิร์นได้แม่นยำ (เผื่อลูกค้าพิมพ์เว้นวรรค)
    text_cleaned = text.lower().replace(" ", "")
    
    # 2. [CORE LOGIC] กรองเฉพาะข้อความที่มีรูปแบบรหัสเท่านั้น
    # Pattern: ขึ้นต้นด้วย 269 หรือ 999 ตามด้วยตัวอักษร/ตัวเลข 6 ตัว
    # รวมความยาว = 9 ตัวอักษร
    pattern = r'(?:269|999)[a-z0-9]{6}'
    
    # ค้นหารหัสทั้งหมดที่ตรงตาม Pattern ในข้อความ
    valid_format_codes = re.findall(pattern, text_cleaned)
    
    # 🛑 GATEKEEPER: ถ้าไม่เจอสิ่งที่ดูเหมือนรหัสเลย -> จบการทำงานทันที (เงียบ)
    if not valid_format_codes:
        print(f"   (Ignored) No valid code pattern found in: {text}")
        return

    # --- ถ้าผ่านด่านมาได้ แปลว่ามีรหัสที่ต้องเช็ค ---
    found_actions = [] 
    unknown_codes = []

    for code in valid_format_codes:
        # เช็คว่ารหัสนี้ มีไฟล์อยู่จริงไหม?
        if code in CACHED_FILES:
            ...
            # รูปแบบถูก แต่ไม่มีไฟล์ (เช่น 999xxxxxx ที่ยังไม่อัปโหลด)
